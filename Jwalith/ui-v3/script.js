...
if (filterTab && locationTab && filterPanel && locationPanel) {
    filterTab.addEventListener('click', () => switchSearchTab('filters'));
    locationTab.addEventListener('click', () => switchSearchTab('location'));
}
...
function createResultCard(org) {
    ...
    const distanceInfo = org.distance ? `
        <div class="card-info">
            ...
            <span class="card-info-text"><strong>Distance:</strong> ${org.distance} miles away</span>
        </div>
    ` : '';
    
    card.innerHTML = `
        <div class="card-header">
            <h3 class="card-title">${escapeHtml(org.name)}</h3>
            <span class="card-type">${escapeHtml(org.type)}</span>
        </div>
        <div class="card-body">
            ${distanceInfo}
            <div class="card-info">
                <span class="card-info-icon">📍</span>
                <span class="card-info-text">${escapeHtml(org.city)}, ${escapeHtml(org.state)} ${org.zip || ''}</span>
            </div>
            ${org.address ? `
            <div class="card-info">
                <span class="card-info-icon">🏠</span>
                <span class="card-info-text">${escapeHtml(org.address)}</span>
            </div>
            ` : ''}
            ${org.phone ? `
            <div class="card-info">
                ...
                <span class="card-info-text">
                    <a href="tel:${org.phone}" class="card-info-link" title="Click to call">
            ...
            <div class="card-info">
                ...
                <span class="card-info-text">
                    <a href="mailto:${org.email}" class="card-info-link" title="Click to send email">
